...
static void on_volume_removed(gchar* output, gpointer user_data) {
    RemoveVolumeData* data = (RemoveVolumeData*)user_data;
    ...
    if (data && data->store) {
        refresh_volumes_table_async(data->store);
    }
    if (data) {
        g_free(data->volume_name);
        g_free(data);
    }
    if (output) {
        g_free(output);
...
static void on_confirm_volume_dialog_response(GtkDialog* dialog, gint response_id, gpointer user_data) {
    RemoveVolumeData* confirm_data = (RemoveVolumeData*)user_data;
...
static void on_volume_inspect_complete(gchar* output, gpointer user_data) {
    InspectVolumeData* data = (InspectVolumeData*)user_data;
...
static void on_inspect_volume_clicked(GtkMenuItem* menu_item, gpointer user_data) {
    RemoveVolumeData* data = (RemoveVolumeData*)user_data;
    ...
    if (!volume_name || strlen(volume_name) == 0) {
        g_free(volume_name);
        return;
    }
...
static void on_remove_volume_clicked(GtkMenuItem* menu_item, gpointer user_data) {
    ...
    confirm_data->volume_name = g_strdup(volume_name);
    g_signal_connect(dialog, "response", G_CALLBACK(on_confirm_volume_dialog_response), confirm_data);
    gtk_widget_show_all(dialog);
...
GtkWidget* create_volumes_table(void) {
    ...
    column = gtk_tree_view_column_new_with_attributes(
        "DRIVER", renderer, "text", 1, NULL);
    gtk_tree_view_column_set_resizable(column, TRUE);
    gtk_tree_view_column_set_sort_column_id(column, 1);
    gtk_tree_view_append_column(GTK_TREE_VIEW(tree_view), column);
    renderer = gtk_cell_renderer_text_new();
    column = gtk_tree_view_column_new_with_attributes(
        "SCOPE", renderer, "text", 2, NULL);
    gtk_tree_view_column_set_resizable(column, TRUE);
    gtk_tree_view_column_set_sort_column_id(column, 2);
    gtk_tree_view_append_column(GTK_TREE_VIEW(tree_view), column);
    renderer = gtk_cell_renderer_text_new();
    column = gtk_tree_view_column_new_with_attributes(
        "MOUNTPOINT", renderer, "text", 3, NULL);
    gtk_tree_view_column_set_resizable(column, TRUE);
    gtk_tree_view_column_set_sort_column_id(column, 3);
    gtk_tree_view_append_column(GTK_TREE_VIEW(tree_view), column);
    GtkWidget* context_menu = gtk_menu_new();
    RemoveVolumeData* menu_data = g_new(RemoveVolumeData, 1);
